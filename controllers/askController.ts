...
const ask_all_get = (req: Express.Request, res: Express.Response) => {
  ...
  const { categories, showHidden }: { categories?: string, showHidden?: boolean } = req.query;
  const categoryList = categories ? categories.split(",").map(item => item.trim()) : [];
  ...
  if (categoryList.length > 0) {
    ...
    Ask.find({ $and: [{ categories: { $in: categoryList } }, { "status.hidden": { $eq: false } }] }).sort({ createdAt: -1 })
